...
public:
    ...
    // 返回fd当前的事件状态
    bool isNoneEvent() const { return events_ == kNoneEvent; }  // 是否关闭了所有事件
    bool isWriting() const { return events_ & kWriteEvent; }  // 是否关注了写事件
    bool isReading() const { return events_ & kReadEvent; }  // 是否关注了读事件
...
private:
    ...
    bool tied_;  // 标志此 Channel 是否与TcpConnection对象关联，即被调用过 Channel::tie 方法

    // 因为 channel 通道里面能够获知fd最终发生的具体的事件revents，所以它负责调用具体事件的回调操作
    // 保存事件到来时的回调函数
