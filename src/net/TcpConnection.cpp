...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    // 说明一次性并没有发送完数据，剩余数据需要保存到缓冲区中，且需要给channel注册epollout写事件，
    // 然后poller发现tcp的发送缓冲区有空间，会通知相应的sock-channel，调用handleWrite回调方法
...
void TcpConnection::connectEstablished()
{
     ...
     * channel->tie 会进行一次判断，是否将弱引用指针变成强引用，变的成话就防止了计数为0而被析构的可能
     */
    channel_->tie(shared_from_this());
...
// 当服务端接收客户端数据，EventLoop 返回活跃的 Channel，并调用对应的读事件处理函数，
// 即 TcpConnection 调用 handleRead 方法从相应的 fd 中读取数据到 inputBuffer 中。
// 在 Buffer 内部 inputBuffer 中的 writeIndex 向后移动。
/**
 * 消息读取，TcpConnection从客户端读取数据（从客户端socket读取数据到inputBuffer）
 * 调用Buffer.readFd(fd, errno) -> 内部调用readv将数据从fd读取到缓冲区 -> inputBuffer
 */
void TcpConnection::handleRead(Timestamp receiveTime)
{
    int savedErrno = 0;
    // TcpConnection会从socket读取数据，然后写入inputBuffer
    // 将 channel_->fd() 数据读取到 inputBuffer_ 中，出错信息保存到 savedErrno 中
    ssize_t n = inputBuffer_.readFd(channel_->fd(), &savedErrno);
    if (n > 0) {  // 已建立连接的用户，有可读事件发生，调用用户传入的回调操作onMessage
        // shared_from_this就是获取了TcpConnection的智能指针
        messageCallback_(shared_from_this(), &inputBuffer_, receiveTime);
    } else if (n == 0) {  // 没有数据，说明客户端关闭连接
        handleClose();
    } else {  // 出错情况
        errno = savedErrno;
...
// 当服务端向客户端发送数据，TcpConnection 调用 handleWrite 方法将 outputBuffer 的数据写入到 TCP 发送缓冲区。
// outputBuffer 内部调用 retrieve 方法移动 readIndex 索引。
/**
 * 消息发送，TcpConnection向客户端发送数据（将ouputBuffer数据输出到socket中）
 */
void TcpConnection::handleWrite()
{
    // 要在channel_确实关注写事件的前提下正常发送数据
    // 因为一般有一个send函数发送数据，如果TCP接收缓冲区不够接收ouputBuffer的数据，就需要多次写入。
    // 需要重新注册写事件，因此是在注册了写事件的情况下调用的handleWrite。
    if (channel_->isWriting()) {
        int saveErrno = 0;
        // 向channel->fd()发送outputBuffer中的可读取数据
        ssize_t n = outputBuffer_.writeFd(channel_->fd(), &saveErrno);
        // 正确读取数据
        if (n > 0) {
            outputBuffer_.retrieve(n);
            // 说明buffer可读数据都被TcpConnection读取完毕并写入给了客户端
            // 此时就可以关闭连接，否则还需继续关注写事件
