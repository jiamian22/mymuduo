 ...
 * Buffer_空间如果不够会读入到栈上65536个字节大小的空间，然后以append的方式追加入buffer_。
 * 既考虑了避免系统调用带来开销，又不影响数据的接收。
 **/
ssize_t Buffer::readFd(int fd, int* saveErrno)
{
    // 栈额外空间，用于从套接字往出读时，当buffer_暂时不够用时暂存数据，待buffer_重新分配足够空间后，再把数据交换给buffer_。
    ...
    */
    ...
    if (n < 0) {
        *saveErrno = errno;
        ...
        writerIndex_ += n;
    } else {  // buffer可写数据空间不够，还需写入extrabuf，extrabuf里面也写入了n-writable长度的数据
        writerIndex_ = buffer_.size();  // writerIndex直接到尾部
